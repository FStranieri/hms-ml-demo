...
class LocalImageClassificationTransactor(private val mContext: Context) :
    ...
    override fun stop() {
        try {
            ...
            Log.e(
                TAG,
                "Exception thrown while trying to close image classification transactor: " + e.message
            )
        }
    }
    ...
    override fun onSuccess(
        ...
        val hmsMLLocalImageClassificationGraphic =
            LocalImageClassificationGraphic(graphicOverlay, mContext, classifications)
    ...
    companion object {
        private const val TAG = "ClassificationTrans"
    }

    init {
        detector = MLAnalyzerFactory.getInstance().localImageClassificationAnalyzer
    }
}