...
class SceneDetectionTransactor : BaseTransactor<List<MLSceneDetection>> {
    private val analyzer: MLSceneDetectionAnalyzer
    private var setting: MLSceneDetectionAnalyzerSetting? = null
    private var mContext: Context
    private var confidence = 0f

    constructor(context: Context, confidence: Float) {
    ...
    override fun detectInImage(image: MLFrame?): Task<List<MLSceneDetection>> {
        return analyzer.asyncAnalyseFrame(image)
    }

    override fun onSuccess(
        originalCameraImage: Bitmap?,
        results: List<MLSceneDetection>,
        ...
        val graphic = SceneDetectionGraphic(graphicOverlay, mContext, results, confidence)
        graphicOverlay.addGraphic(graphic)
        graphicOverlay.postInvalidate()
    }

    override fun onFailure(e: Exception) {
        Log.e(TAG, "XXX failed: " + e.message)
    }

    companion object {
        private const val TAG = "scdTransactor"
    }
}