...
class LocalImageClassificationGraphic(
    private val overlay: GraphicOverlay,
    private val mContext: Context,
    private val classifications: List<MLImageClassification>
) : BaseGraphic(
    overlay
) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        var y = overlay.height / 1.5f
        for (classification in classifications) {
            canvas.drawText(classification.name, x, y, textPaint)
            y = y - CommonUtils.dp2px(mContext, 18f)
        }
    }
    ...
    init {
        ...
        textPaint.textSize = CommonUtils.dp2px(mContext, 16f)
    }
}