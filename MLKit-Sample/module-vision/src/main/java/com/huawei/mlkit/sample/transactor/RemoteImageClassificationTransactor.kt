...
class RemoteImageClassificationTransactor(context: Context, handler: Handler) :
    BaseTransactor<List<MLImageClassification>>() {
    private val detector: MLImageClassificationAnalyzer
    ...
    override fun stop() {
        ...
        try {
            ...
            Log.e(
                TAG,
                "Exception thrown while trying to close remote image classification transactor" + e.message
            )
        }
    }

    override fun detectInImage(image: MLFrame?): Task<List<MLImageClassification>> {
        return detector.asyncAnalyseFrame(image)
    }

    override fun onSuccess(
        originalCameraImage: Bitmap?,
        classifications: List<MLImageClassification>,
        frameMetadata: FrameMetadata,
        graphicOverlay: GraphicOverlay
    ) {
        graphicOverlay.clear()
        handler.sendEmptyMessage(Constant.GET_DATA_SUCCESS)
        val classificationList: MutableList<String> = ArrayList()
        for (i in classifications.indices) {
            val classification = classifications[i]
            if (classification.name != null) {
                classificationList.add(classification.name)
            }
        }
        val remoteImageClassificationGraphic =
            RemoteImageClassificationGraphic(graphicOverlay, this.mContext, classificationList)
    ...
    override fun onFailure(e: Exception) {
        ...
        Log.e(TAG, "Remote image classification detection failed: " + e.message)
    }

    companion object {
        private const val TAG = "RemoteImgClassification"
    }

    init {
        val options =
            MLRemoteClassificationAnalyzerSetting.Factory().setMinAcceptablePossibility(0f).create()
        detector = MLAnalyzerFactory.getInstance().getRemoteImageClassificationAnalyzer(options)
