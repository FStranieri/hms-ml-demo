...
class RemoteImageClassificationGraphic(
    private val overlay: GraphicOverlay,
    private val mContext: Context,
    private val classifications: List<String>
) : BaseGraphic(
    overlay
) {
    ...
    override fun draw(canvas: Canvas) {
        var x = 0f
        var index = 0
        val space = CommonUtils.dp2px(mContext, 16f)
        var y = overlay.height - CommonUtils.dp2px(mContext, 30f)
        for (classification in classifications) {
            if (classification.length > MAX_LENGTH) {
                canvas.drawText(classification.substring(0, MAX_LENGTH), x, y, textPaint)
                y = y - space
                canvas.drawText(classification.substring(MAX_LENGTH), x, y, textPaint)
            } else {
                index++
                if (index == 1) {
                    x = CommonUtils.dp2px(mContext, 12f)
                } else if (index == 2) {
                    x = (overlay.width / 2).toFloat()
                }
                canvas.drawText(classification, x, y, textPaint)
                if (index == 2) {
                    y = y - space
                    index = 0
                }
    ...
    init {
        ...
        textPaint.textSize = CommonUtils.dp2px(mContext, 14f)
    }
}