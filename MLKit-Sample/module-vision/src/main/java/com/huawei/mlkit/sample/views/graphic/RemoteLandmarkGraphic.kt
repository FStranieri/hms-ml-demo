...
class RemoteLandmarkGraphic(
    private val overlay: GraphicOverlay,
    private val landmark: MLRemoteLandmark
) : BaseGraphic(
    overlay
) {
    ...
    override fun draw(canvas: Canvas) {
        val boundingBox = landmark.border
        if (null != boundingBox) {
        ...
        val x = 0
        val y = overlay.height / 2
        canvas.drawText(landmark.landmark, x.toFloat(), y.toFloat(), textPaint)
        if (landmark.positionInfos == null || landmark.positionInfos.isEmpty()) {
            canvas.drawText("Unknown location", x.toFloat(), (y + 50).toFloat(), textPaint)
        } else {
            for (location in landmark.positionInfos) {
                canvas.drawText(
            ...
            canvas.drawText(
                "confidence: " + landmark.possibility,
                x.toFloat(),
                (y + 150).toFloat(),
                textPaint
            )
